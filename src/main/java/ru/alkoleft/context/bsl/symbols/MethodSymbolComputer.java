...
public class MethodSymbolComputer extends BSLParserBaseVisitor<ParseTree> {
  ...
  private static List<ParameterSymbol> createParameters(BSLParser.ParamListContext paramList,
    ...
    return paramList.param().stream()
        .map((BSLParser.ParamContext param) -> {
          ...
          return ParameterSymbol.builder()
              .name(parameterName)
  ...
  private static ParameterSymbol.DefaultValue getDefaultValue(BSLParser.ParamContext param) {
    if (param.defaultValue() == null) {
      return ParameterSymbol.DefaultValue.EMPTY;
    }

    var constValue = param.defaultValue().constValue();

    ParameterSymbol.DefaultValue defaultValue;
    if (constValue.DATETIME() != null) {
      var value = constValue.DATETIME().getSymbol().getText();
      defaultValue = new ParameterSymbol.DefaultValue(ParameterSymbol.ParameterType.DATETIME, value.intern());
    } else if (constValue.FALSE() != null) {
      var value = constValue.FALSE().getSymbol().getText();
      defaultValue = new ParameterSymbol.DefaultValue(ParameterSymbol.ParameterType.BOOLEAN, value.intern());
      ...
      defaultValue = new ParameterSymbol.DefaultValue(ParameterSymbol.ParameterType.UNDEFINED, value.intern());
    } else if (constValue.NULL() != null) {
      var value = constValue.NULL().getSymbol().getText();
      defaultValue = new ParameterSymbol.DefaultValue(ParameterSymbol.ParameterType.NULL, value.intern());
    } else if (constValue.string() != null) {
      var value = constValue.string().STRING().stream()
          .map(TerminalNode::getSymbol)
          .map(Token::getText)
          .collect(Collectors.joining("\n"));
      defaultValue = new ParameterSymbol.DefaultValue(ParameterSymbol.ParameterType.STRING, value.intern());
      ...
      defaultValue = new ParameterSymbol.DefaultValue(ParameterSymbol.ParameterType.NUMERIC, value.intern());
    } else {
      defaultValue = ParameterSymbol.DefaultValue.EMPTY;
    }
